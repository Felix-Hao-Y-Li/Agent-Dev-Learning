"""
RAG Part 2 自测题 —— 练习版（挖空）

一共 6 处 TODO，每处只需要写 1-3 行。其余代码全部写好，直接能跑。
每个 TODO 都配三层信息：目标 / 提示① 方向 / 提示② 接近伪代码。
先自己想，卡住了看提示，还写不出来就去 rag_05_answers.py 对答案——
两份文件除 TODO 处之外逐字一致，可以直接 diff。

怎么做：
    1. 搜索 "TODO(你来写)" 找到 6 个空位
    2. 一次填一个，填完就跑，看输出对不对
    3. 没填的空位会给出中文提示并停下，不会抛难懂的报错

运行方式：
    uv run rag_05_selftest.py            # 跑全部三题
    uv run rag_05_selftest.py 1a 2 3     # 只跑指定的题（题 1b 需要额外下载模型）
...
# 脚手架区：这一段整段都可以跳过
# ===========================================================================
def _require(value, tag: str):
    """[脚手架] 未填空时给出中文提示，而不是让程序在好几层之后抛难懂的报错。"""
    if value is None:
        raise NotImplementedError(
            f"\n\n  {tag} 还没填。\n"
            f"  请在本文件里搜索 '{tag}'，按上面的提示写完那一行再运行。\n"
            f"  实在写不出来，答案在 rag_05_answers.py 的同名位置。\n"
        )
    return value
...
def test_1_model_mismatch(chunks: list[Document], run_1b: bool = True) -> None:
    ...
    # ---- 1A：同一个模型，但查询端配置不同 --------------------------------
    # ---- 题 1-① ----------------------------------------------------------
    # 目标：造一个「和 emb_ref 同模型、但查询端不加前缀」的嵌入模型对象。
    #
    # 提示① 上面的 build_embeddings() 已经有一个参数专门控制加不加前缀，
    #        回去看看它的签名，默认值是什么？你要传的是相反的那个值。
    #
    # 提示② 就一行，形如：
    #        emb_no_prefix = build_embeddings(参数名=False)
    #
    # 答案在 rag_05_answers.py 的 test_1_model_mismatch()
    emb_no_prefix = None        # TODO(你来写)
    # ---------------------------------------------------------------------
    _require(emb_no_prefix, "题 1-①")
    ...
    # ---- 题 1-② ----------------------------------------------------------
    # 目标：造一个用 OTHER_MODEL（384 维）的嵌入模型对象，拿它去加载 512 维的索引。
    #
    # 先在下面这行注释里写下你的预测，再跑：
    #     我预测会：______________________________
    #
    # 提示① 还是用 build_embeddings()，但这次要换的不只是前缀，还有模型名。
    #        它的第二个参数就是干这个的，默认值是 MODEL_NAME。
    #
    # 提示② 一行，两个参数：
    #        emb_384 = build_embeddings(with_prefix=False, 参数名=OTHER_MODEL)
    #
    # 答案在 rag_05_answers.py 的 test_1_model_mismatch()
    emb_384 = None              # TODO(你来写)
    # ---------------------------------------------------------------------
    _require(emb_384, "题 1-②")
    try:
...
def stable_id(chunk: Document) -> str:
    """[核心] 为一个块生成稳定不变的 id。"""
    # ---- 题 2-① ----------------------------------------------------------
    # 目标：把一个 chunk 变成稳定不变的字符串 id。
    #
    # 提示① 什么叫「稳定」？同一块文本跑两次要得到完全一样的 id。
    #        所以不能用随机数、不能用时间戳、也不能用列表下标
    #        （切分参数一改，第 3 块就不再是原来那一块了）。
    #        chunk.metadata 里有哪些字段是内容决定的、不会变的？打印出来看看。
    #
    # 提示② 先试最直觉的写法：
    #            return f"{meta['source']}#{meta['start_index']}"
    #        跑一下题 2，你会撞到一个报错。看清楚它说什么，
    #        然后回想第 1 课关于 start_index 的那条结论——
    #        它是相对于「传给切分器的那个 Document」的偏移，不是相对于整个文件。
    #        我们是先按标题切、再对每块递归切的，所以会发生什么？
    #        补一个能区分开的字段进去（metadata 里的 h1/h2/h3）。
    #        小心：不是每个块都有 h3，用 .get(key, "") 而不是 meta[key]。
    #
    # 答案在 rag_05_answers.py 的 stable_id()
    meta = chunk.metadata
    raise NotImplementedError(
        "\n\n  题 2-① 还没填：stable_id() 需要你返回一个稳定的字符串 id。\n"
        "  看上面的提示①②，答案在 rag_05_answers.py 的 stable_id()。\n"
    )
    # ---------------------------------------------------------------------
...
def _run_three_rounds(chunks: list[Document], emb: HuggingFaceEmbeddings,
    ...
    for round_no in (1, 2, 3):
        ...
        if use_stable_ids:
            ...
            # 目标：入库时把上面那个稳定 id 用上。
            #
            # 提示① 对比一下 else 分支里那行 store.add_documents(chunks)——
            #        它没告诉 Chroma 用什么 id，所以 Chroma 自己编了随机 uuid。
            #        add_documents 有个参数可以指定 id，去查一下叫什么。
            #
            # 提示② 用列表推导式给每个 chunk 算一个 id 传进去：
            #        store.add_documents(chunks, 参数名=[stable_id(c) for c in chunks])
            #
            # 答案在 rag_05_answers.py 的 _run_three_rounds()
            raise NotImplementedError(
                "\n\n  题 2-② 还没填：这一行要把稳定 id 传给 add_documents。\n"
                "  答案在 rag_05_answers.py 的 _run_three_rounds()。\n"
            )
            # -------------------------------------------------------------
        else:
...
def pairwise_mean_similarity(docs: list[Document], emb: HuggingFaceEmbeddings) -> float:
    ...
    # ---- 题 3-① ----------------------------------------------------------
    # 目标：给定 k 条文档，返回它们两两之间余弦相似度的平均值。
    #
    # 提示① 拆成三步想：
    #        ① 怎么把这几条文本变成向量？（emb 上有个方法一次吃一个字符串列表）
    #        ② 3 条文档有几对组合？(0,1) (0,2) (1,2) —— 文件顶部 import 了
    #           itertools.combinations，它就是干这个的：combinations(range(n), 2)
    #        ③ 每对算一次 cosine()（本文件已有这个函数），再求平均
    #
    # 提示② 骨架长这样，三行：
    #        vecs = [np.array(v) for v in emb.embed_documents([...每条的 page_content...])]
    #        pairs = list(combinations(range(len(vecs)), 2))
    #        return sum(cosine(vecs[i], vecs[j]) for i, j in pairs) / len(pairs)
    #
    # 答案在 rag_05_answers.py 的 pairwise_mean_similarity()
    raise NotImplementedError(
        "\n\n  题 3-① 还没填：pairwise_mean_similarity() 要返回一个 float。\n"
        "  答案在 rag_05_answers.py 的同名函数。\n"
    )
    # ---------------------------------------------------------------------
...
def test_3_mmr_diversity(chunks: list[Document], emb: HuggingFaceEmbeddings) -> None:
    ...
    # ---- 题 3-② ----------------------------------------------------------
    # 目标：让 lambda_mult 从 0.1 扫到 0.9（共 9 个取值）。
    #
    # 提示① lambda_mult 越接近 1 越看重相关性，越接近 0 越看重多样性。
    #        不要用 while lam < 1: lam += 0.1 这种写法——
    #        浮点数累加有精度误差，0.1+0.1+0.1 在计算机里不等于 0.3。
    #
    # 提示② 先生成整数再除：
    #        lambdas = [i / 10 for i in range(1, ??)]
    #
    # 答案在 rag_05_answers.py 的 test_3_mmr_diversity()
    lambdas = None              # TODO(你来写)
    # ---------------------------------------------------------------------
    for lam in _require(lambdas, "题 3-②"):
